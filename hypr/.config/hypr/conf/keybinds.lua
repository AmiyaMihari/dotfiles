...
-- (el repo original tenia binds a layout.sh/layout2.sh y a un emoji picker
-- basado en dmenu+xclip, que son herramientas de X11 y no vienen en este
-- setup ni funcionan bien bajo Wayland; se omiten)
...
hl.bind(mainMod .. " + S", hl.dsp.exec_cmd([[bash -c 'f="${HOME}/Pictures/screenshots/SS$(date +%Y-%m-%d_%H-%M-%S).png"; grim "$f" && wl-copy --type image/png < "$f"']]))

-- screenshot + crop keybind
hl.bind(mainMod .. " + SHIFT + S", hl.dsp.exec_cmd([[bash -c 'f="${HOME}/Pictures/screenshots/SS$(date +%Y-%m-%d_%H-%M-%S).png"; grim -g "$(slurp)" "$f" && wl-copy --type image/png < "$f"']]))
